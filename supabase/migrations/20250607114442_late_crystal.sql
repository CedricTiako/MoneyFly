/*
  # Données de test pour tiako1998@gmail.com
  
  1. Budget mensuel : 400 000 FCFA
  2. Catégories détaillées avec sous-catégories
  3. Dépenses réalistes sur 3 mois
  4. Objectifs d'épargne
  5. Tontines familiales
*/
...
BEGIN
  ...
  VALUES (gen_random_uuid(), user_uuid, '2024-12', 400000, 385000, 150000)
  RETURNING id INTO budget_id_current;
  
  -- Budget précédent (Novembre 2024)
  INSERT INTO budgets (id, user_id, mois, revenu, total_depense, total_epargne) 
  VALUES (gen_random_uuid(), user_uuid, '2024-11', 400000, 392000, 150000)
  RETURNING id INTO budget_id_prev;
  
  -- Budget d'avant (Octobre 2024)
  INSERT INTO budgets (id, user_id, mois, revenu, total_depense, total_epargne) 
  VALUES (gen_random_uuid(), user_uuid, '2024-10', 400000, 378000, 150000)
  RETURNING id INTO budget_id_prev2;

  -- 3. CRÉER LES DÉPENSES DÉTAILLÉES (Décembre 2024)
  
  -- ÉPARGNE (150 000 FCFA)
  INSERT INTO depenses (user_id, budget_id, categorie_id, montant, description, date_depense) VALUES
  (user_uuid, budget_id_current, cat_epargne, 150000, 'Épargne mensuelle - Objectif 2M/an', '2024-12-01');
  ...
  -- 4. DÉPENSES NOVEMBRE 2024 (quelques exemples)
  INSERT INTO depenses (user_id, budget_id, categorie_id, montant, description, date_depense) VALUES
  (user_uuid, budget_id_prev, cat_epargne, 150000, 'Épargne mensuelle', '2024-11-01'),
  ...
  (user_uuid, budget_id_prev, cat_divers, 22000, 'Imprévus et urgences', '2024-11-25');

  -- 5. DÉPENSES OCTOBRE 2024 (quelques exemples)
  INSERT INTO depenses (user_id, budget_id, categorie_id, montant, description, date_depense) VALUES
  (user_uuid, budget_id_prev2, cat_epargne, 150000, 'Épargne mensuelle', '2024-10-01'),
  ...
  (user_uuid, budget_id_prev2, cat_divers, 18000, 'Imprévus', '2024-10-25');
...
SELECT 
  ...
  b.total_depense,
  b.total_epargne,
  (b.revenu - b.total_depense - b.total_epargne) as restant
FROM budgets b 
WHERE b.user_id = (SELECT id FROM auth.users WHERE email = 'tiako1998@gmail.com')
